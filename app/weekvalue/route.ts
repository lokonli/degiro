...
export async function GET(request: Request) {
  ...
  if (n === 0) {
    const svg = renderSvgLineChart([0, 0], {
      width,
      height,
      padding,
      color: colorOverride ?? GAIN_COLOR,
      background,
      strokeWidth,
      fill,
    });
  ...
  const windowed = series.portfolioValue.slice(Math.max(0, n - days));
  const start = windowed[0];
  const deltas = windowed.map((v) => v - start);
  const color = colorOverride ?? (deltas[deltas.length - 1] >= 0 ? GAIN_COLOR : LOSS_COLOR);

  const svg = renderSvgLineChart(deltas, { width, height, padding, color, background, strokeWidth, fill });
